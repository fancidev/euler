...
struct euler_problem_info
{
	...
	PROBLEM_FUNC routine;        /**< Entry of the solution routine. */
	const char *title;           /**< Problem title. */
	int difficulty;              /**< Difficulty of the problem (on a scale of 1 to 3). */
	int fun_level;               /**< Fun level of the problem (on a scale of 1 to 3). */
	const char *time_complexity; /**< Time complexity of the solution. */
	const char *space_complexity;/**< Space complexity of the solution. */
	const char *answer;          /**< Answer to the problem. */
	const char *keywords;        /**< Keywords separated by comma. */

public:
	/// Constructs an empty object.
	euler_problem_info() : id(0), routine(NULL), title(NULL), difficulty(0),
		fun_level(0), time_complexity(NULL), space_complexity(NULL),
		answer(NULL), keywords(NULL) { }
};
