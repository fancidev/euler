...
bool miller_rabin(T n, std::false_type /* is_signed */)
{
  ...
  static const uint64_t threshold[4] = {
    //9080191ULL,
    4759123141ULL,
    2152302898747ULL,
    3474749660383ULL,
  ...
  for (int i = 0; candidates[k][i] != 0 && candidates[k][i] < n; i++)
  {
    if (!miller_rabin(n, s, d, static_cast<T>(candidates[k][i])))
    {
      return false;
    }
  }
...
bool miller_rabin(TArg n, std::true_type /* is_signed */)
{
  ...
  {
    return miller_rabin(
        static_cast<typename std::make_unsigned<TArg>::type>(n),
        std::false_type());
  }
}
