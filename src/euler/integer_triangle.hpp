...
// Generate primitive Pythagorean triangles (i.e. integer-sided right triangles).
// The algorithm runs a nested loop. In the inner loop, for each triangle generated, 
// it calls function f(a,b,c)->bool. If f returns false, the inner loop is 
// terminated. If f returns false for the first iteration of the inner loop,
...
// Generate primitive, integer-sided triangles with exactly one 60-degree angle.
// The algorithm runs a nested loop. In the inner loop, for each triangle generated, 
// it calls function f(a,b,c)->bool. If f returns false, the inner loop is 
// terminated. If f returns false for the first iteration of the inner loop,
// the outer loop is terminated.
...
// Generate primitive, integer-sided triangles with one 120-degree angle.
// The algorithm runs a nested loop. In the inner loop, for each triangle generated, 
// it calls function f(a,b,c)->bool. If f returns false, the inner loop is 
// terminated. If f returns false for the first iteration of the inner loop,
// the outer loop is terminated.
// Note: it is guaranteed that a is the longest side.
template <typename integer_t, typename Func>
